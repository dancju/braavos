...
export class EthereumService extends ChainService {
  protected readonly hdkey: BIP32Interface;
  private readonly web3: Web3;

  constructor(config: ConfigService, web3: Web3) {
    super();
    const seed = config.seed;
    this.hdkey = fromSeed(seed);
    this.web3 = web3;
  }

  public async getAddr(clientId: number, path0: string): Promise<string> {
    const path1 = `m/44'/60'/0'/${clientId}/${path0}`;
    const addr = this.web3.eth.accounts.privateKeyToAccount(
      this.hdkey.derivePath(path1).privateKey!.toString('hex'),
    ).address;
  ...
  public getPrivateKey(clientId: number, path0: string): string {
    const path1 = `m/44'/60'/0'/${clientId}/${path0}`;
    return this.hdkey.derivePath(path1).privateKey!.toString('hex');
  }
}
